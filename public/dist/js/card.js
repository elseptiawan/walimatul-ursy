...
export const card = (() => {
    ...
    const renderTitle = (comment, is_parent) => {
        ...
        if (is_parent) {
            return `<strong class="me-1">${util.escapeHtml(comment.user.name)}</strong><i class="fa-solid ${comment.is_present ? 'fa-circle-check text-success' : 'fa-circle-xmark text-danger'}"></i>`;
        }

        return `<strong>${util.escapeHtml(comment.user.name)}</strong>`;
    };
    ...
    const renderBody = (comment, is_parent) => {
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            ...
            <small class="text-${text} m-0 p-0" style="font-size: 0.75rem;">${date}</small>
        </div>
